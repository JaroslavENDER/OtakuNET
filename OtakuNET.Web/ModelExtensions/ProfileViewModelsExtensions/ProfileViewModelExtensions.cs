...
namespace OtakuNET.Web.ModelExtensions.ProfileViewModelsExtensions
{
    public static class ProfileViewModelExtensions
    {
        public static ProfileViewModel Initialize(this ProfileViewModel model, Profile profile, ITimestampFormatter timestampFormatter)
        {
            ...
            model.UserAnimeLists = profile.UserListSet.Where(ul => ul.Type == TitleType.Anime).Select(l => new UserListInfoViewModel().Initialize(l)).ToList();
            model.UserMangaLists = profile.UserListSet.Where(ul => ul.Type == TitleType.Manga).Select(l => new UserListInfoViewModel().Initialize(l)).ToList();
